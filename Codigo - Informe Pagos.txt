...
fecha_actual_cierre="2022-10-01" #MES ACTUAL PARA SACAR LA COLUMNA DE CATEGORIA DE PAGO(mayor al primer dia del mes)
...
#ORDENAMOS LA INFORMACION DE MANERA CORRECTA
conexion1 <- conexion1[with(conexion1, order(conexion1$cedula_referido)), ]
conexion1 <- conexion1[with(conexion1, order(-conexion1$referido_fecha_ing)), ]
...
#    archivo_sucursal,
#    final_formato_abono_estado_de_cuenta,
#    formato_pxl_abono_estado_cuenta,
#    tabla_formato_pxl_abono_estado_cuenta,
#    final_formato_giros_y_consignaciones,
#    liquidacion_financiera)
...
archivo_referidos <- read_csv(
  files_referidos[[1]],
  locale = locale(encoding = "ISO-8859-1"),
  col_types = cols(.default = col_character()))%>% 
  clean_names() %>% 
  rename(cedula_referido=referido_id_personal,
         cedula_asociado=tinker_id_personal,
         nombre_asociado=tinker_nombre,
         telefono_asociado=tinker_telefono,
         correo_asociado=tinker_correo,
         fecha_registro_referido=referido_fecha_registro,
         nombre_referido=referido_nombre,
         telefono_referido=referido_telefono,
         correo_referido=referido_correo,
         metodo_de_pago_asociado=tinker_medio_de_pago,
         ciudad_sucursal_de_pago_asociado=tinker_sucursal) %>% 
  ...
  mutate(fecha_registro_referido = dmy_hm(fecha_registro_referido),
         fecha_registro_referido = as.Date(fecha_registro_referido),
         cedula_referido=as.character(cedula_referido),
         cedula_asociado=as.character(cedula_asociado),
         tipo_cuenta_asociado="nueva herramienta",
         numero_de_cuenta_asociado="nueva herramienta",
         entidad_financiera_asociado="nueva herramienta")
...
# )
# 
# archivo_referentes <- read_csv(
#   files_referentes[[1]],
#   locale = locale(encoding = "ISO-8859-1"),
#   col_types = cols(.default = col_character())) %>% 
#   clean_names() %>% 
#   rename(cedula_asociado=tinker_id_personal,
...
tabla_conteo_referidos_vinculados_inicio <- pagos_red_cooperamos %>% 
  filter(fecha_registro_referido>=fecha_inicio,referido_fecha_ing>=fecha_inicio) %>% #referido_efectivo=="Si" QUITE ESTE FILTRO PORQUE LAS EXEPCIONES NO TODAS SALEN REFERIDO_EFECTIVO="SI"
  select(cedula_asociado) %>% 
...
#QUITAMOS LOS ESPACIOS EN BLANCO EN CADA UNA DE LAS CELDAS
pagos_red_cooperamos$estado_asociado <- trim(pagos_red_cooperamos$estado_asociado)
...
tabla_conteo_referidos_vinculados_mes_actual <- pagos_red_cooperamos %>%
  filter(referido_fecha_ing>=fecha_actual_cierre) %>% #QUITE REFERIDO EFECTIVO referido_efectivo=="Si",
  select(cedula_asociado) %>%
  count(cedula_asociado) %>%
...
#COLUMNA REPRESENTANTE LEGAL CORRETAJE ------------------------------------------------------------------------------
archivo_cedulas_rl_corretaje <- read_xlsx(path = "./raw_data/cedulas_representantes_legales_corretaje.xlsx") %>% 
  clean_names() %>% 
  select(cedula) %>% 
  mutate(representante_legal_corretaje="Si",
         cedula=as.character(cedula)) 


pagos_red_cooperamos %<>%
  left_join(archivo_cedulas_rl_corretaje,by=c("cedula_asociado"="cedula"))

#REEMPLAZAR NA POR NO
pagos_red_cooperamos$representante_legal_corretaje[is.na(pagos_red_cooperamos$representante_legal_corretaje)]="No"


#COLUMNA CORRESPONDE PREMIO AL ASOCIADO-----------------------------------------------------------------------
pagos_red_cooperamos %<>%
  mutate(corresponde_premio_al_asociado=if_else(referido_efectivo == "Si"&
                                                referido_cuota_pagada >= 1&
                                                referente_fuerza_comercial=="No"&
                                                fecha_registro_referido>fecha_inicio&
                                                fecha_primera_contribuccion!=is.na(fecha_primera_contribuccion)&
                                                representante_legal_corretaje=="No"&
                                                estado_asociado%in%c("Activo Normal","Activo Cobranza Cobranza Interna","Inactivo"),
                                                "Si","No"),
         corresponde_premio_al_asociado=case_when(excepcion_estado=="excepcion"&
                                                  estado_asociado%in%c("Activo Normal","Activo Cobranza Cobranza Interna","Inactivo")~"Si",
                                                  TRUE~corresponde_premio_al_asociado),
         categoria_de_pago=if_else(categoria_nombramiento%in%c("pionero","lider","mentor")&
                                   categoria_de_pago=="sin categoria de pago"&
                                   corresponde_premio_al_asociado=="Si","pionero",categoria_de_pago),
         categoria_de_pago=if_else(categoria_nombramiento%in%c("pionero","lider","mentor")&
                                     categoria_de_pago=="sin categoria de pago"&
                                     corresponde_premio_al_asociado=="Si"&
                                     excepcion_estado=="excepcion","pionero",categoria_de_pago))
...
#COLUMNA sucursal_pago_seleccionado_asociado -----------------------------------------------------------------------

archivo_sucursal <- read_xlsx(path = "./raw_data/codigos.xlsx",
                                               sheet = "codigos_sucursal") %>% 
  clean_names()

pagos_red_cooperamos %<>%
  left_join(archivo_sucursal,by="ciudad_sucursal_de_pago_asociado")
...
pagos_red_cooperamos %<>% 
  ...
  select(indicativo_unico	,
         ...
         colaborador,
         dias,
         representante_legal_corretaje) %>% 
  mutate(estado_de_pago=if_else(corresponde_premio_al_asociado=="Si"&metodo_de_pago_asociado!=""&premio_asociado!="0",
                                "En proceso de pago",""),
         fecha_de_solicitud=if_else(fecha_de_solicitud==is.na(fecha_de_solicitud)&
                                      estado_de_pago!=""|excepcion_estado=="excepcion",hoy,as.Date("")))
...
archivo_consolidado_de_pagos %<>%
  mutate(cedula_referido=as.character(cedula_referido),
         ...
         telefono_referido=as.character(telefono_referido),
         telefono_asociado=as.character(telefono_asociado))
...
# archivo_consolidado_de_pagos2 <- archivo_consolidado_de_pagos %>%
#   mutate(fecha_de_solicitud=as.Date(fecha_de_solicitud)) %>% 
#   filter(fecha_de_solicitud>"01/01/2020",
#          metodo_de_pago_asociado=="Se pago"|metodo_de_pago_asociado=="En proceso de pago")
...
archivo_consolidado_de_pagos %<>% 
  mutate(premio_x_logro=if_else(metodo_de_pago_asociado%in%c("Giro en Oficina Bancoomeva","Consignacion a mi cuenta Bancoomeva","Consignacion a cuentas de otros Bancos","Abono a mi estado de cuenta de Asociado")&
                                cantidad_referidos_vinculados>=4&
                                estado_asociado%in%c("Activo Cobranza Cobranza Interna","Activo Normal")&
                                premio_x_logro=="No","Si","No"))
...
archivo_consolidado_de_pagos %<>%
  ...
  mutate(cuenta_gasto_financiero="5295951000",
         ...
         valor_premio_x_logro=case_when(premio_x_logro=="Si"~"400000",
                                         TRUE~"0"))



#SOLO LE COLOCAMOS FECHA DE SOLICITUD A LO QUE APLIQUE Y EN REALIDAD SE VAYA LIQUIDAR --------------------------------
archivo_consolidado_de_pagos %<>% 
  mutate(fecha_de_solicitud=if_else(fecha_de_solicitud==is.na(fecha_de_solicitud)&
                                      corresponde_premio_al_asociado=="Si"&
                                      metodo_de_pago_asociado=="Abono a mi estado de cuenta de Asociado"&
                                      fecha_corte_asociado==corte&
                                      premio_asociado!=0,hoy,fecha_de_solicitud),
         fecha_de_solicitud=if_else(fecha_de_solicitud==is.na(fecha_de_solicitud)&
                                      corresponde_premio_al_asociado=="Si"&
                                      metodo_de_pago_asociado=="Abono a mi estado de cuenta de Asociado"&
                                      fecha_corte_asociado==corte&
                                      premio_x_logro=="Si",hoy,fecha_de_solicitud),
         fecha_de_solicitud=if_else(fecha_de_solicitud==is.na(fecha_de_solicitud)&
                                      corresponde_premio_al_asociado=="Si"&
                                      metodo_de_pago_asociado%in%c("Giro en oficina Bancoomeva","Consignacion a mi cuenta Bancoomeva","Consignacion a cuentas de otros bancos")&
                                      premio_asociado!=0,hoy,fecha_de_solicitud),
         fecha_de_solicitud=if_else(fecha_de_solicitud==is.na(fecha_de_solicitud)&
                                      corresponde_premio_al_asociado=="Si"&
                                      metodo_de_pago_asociado%in%c("Giro en oficina Bancoomeva","Consignacion a mi cuenta Bancoomeva","Consignacion a cuentas de otros bancos")&
                                      premio_x_logro=="Si",hoy,fecha_de_solicitud),
         fecha_de_solicitud=if_else(fecha_de_solicitud==hoy&
                                    corresponde_premio_al_asociado=="No",as.Date(""),fecha_de_solicitud),
         fecha_de_solicitud_pxl=if_else(corresponde_premio_al_asociado=="Si"&
                                          premio_x_logro=="Si"&
                                          fecha_de_solicitud==hoy,hoy,as.Date("")),
         estado_de_pago=if_else(corresponde_premio_al_asociado=="Si"&
                                  fecha_de_solicitud==hoy,"En proceso de pago",estado_de_pago),
         metodo_de_pago_asociado=if_else(metodo_de_pago_asociado==is.na(metodo_de_pago_asociado)&
                                           corresponde_premio_al_asociado=="Si"&
                                           estado_asociado=="Inactivo","Abono a mi estado de cuenta de Asociado",metodo_de_pago_asociado),
         auxiliar_campana_usc=if_else(is.na(auxiliar_campana_usc),"no",auxiliar_campana_usc),
         canal_de_pago_asociado=case_when(corresponde_premio_al_asociado=="Si"&
                                          fecha_de_solicitud==hoy&
                                          metodo_de_pago_asociado=="Giro en oficina Bancoomeva"~"Operaciones",
                                          
                                          corresponde_premio_al_asociado=="Si"&
                                          fecha_de_solicitud==hoy&
                                          metodo_de_pago_asociado=="Consignacion a mi cuenta Bancoomeva"~"Operaciones",
                                          
                                          corresponde_premio_al_asociado=="Si"&
                                          fecha_de_solicitud==hoy&
                                          metodo_de_pago_asociado=="Abono a mi estado de cuenta de Asociado"~"Operaciones",
                                          
                                          corresponde_premio_al_asociado=="Si"&
                                          fecha_de_solicitud==hoy&
                                          metodo_de_pago_asociado=="Consignacion a cuentas de otros bancos"~"Administrativo",
                                          
                                          corresponde_premio_al_asociado=="Si"&
                                          fecha_de_solicitud==hoy&
                                          colaborador=="colaborador"&
                                          estado_asociado!="Activo Normal"|estado_asociado!="Activo Cobranza Cobranza Interna"&
                                          metodo_de_pago_asociado=="Consignacion a cuentas de otros bancos"~"Administrativo",
                                          TRUE~canal_de_pago_asociado))


## ESTE CODIGO SE DEBE BORRAR CUANDO SE TENGA LA HERRAMIENTA PROPIA --------------------------------------------------
# MEDIOS DE PAGO PARA LAS PERSONAS QUE NO TENGAN CUENTA BANCARIA Y ENTIDAD FINANCIERA--------------------------------


#CARGAMOS EL ARCHIVO DE CODIGOS
archivo_medios_de_pago <- read_xlsx(path = "./raw_data/medios_de_pago.xlsx",
                                               sheet = "medios_de_pago") %>% 
  clean_names() %>% 
  mutate(cedula_asociado=as.character(cedula_asociado))


archivo_consolidado_de_pagos %<>%
  left_join(archivo_medios_de_pago,by="cedula_asociado")

archivo_consolidado_de_pagos %<>%
  mutate(numero_de_cuenta_asociado=case_when(metodo_de_pago_asociado==tipo_de_pago_escogido_eliminar~numero_cuenta_asociado_eliminar,
                                             TRUE~numero_de_cuenta_asociado),
         entidad_financiera_asociado=case_when(metodo_de_pago_asociado==tipo_de_pago_escogido_eliminar~banco_asociado_eliminar,
                                               TRUE~entidad_financiera_asociado))



#--------------------------------------------------------------------------------------------------------------------
# EXPORTAMOS EL CONSOLIDADO DE PAGOS--------------------------------------------------------------------------------

archivo_consolidado_de_pagos %>%
  write_xlsx(
    glue(
      "./raw_data/",
      "consolidado_de_pagos.xlsx"
    )
  )
...
formato_giros_y_consignaciones <- archivo_consolidado_de_pagos %>% 
  filter(fecha_de_solicitud==hoy,
         corresponde_premio_al_asociado=="Si",
         metodo_de_pago_asociado=="Giro en oficina Bancoomeva"|metodo_de_pago_asociado=="Consignacion a mi cuenta Bancoomeva"|metodo_de_pago_asociado=="Consignacion a cuentas de otros bancos",
         premio_asociado!=0,
         auxiliar_campana_usc=="no") %>% 
  rename(total_valor=premio_asociado,
         medio_de_pago=metodo_de_pago_asociado) %>% 
  mutate(observacion="Liquid Normal",
         codigo_oficina=sucursal_pago_seleccionado_asociado) %>% 
...
#---------------------------------------------------------------------------------------------------------------
# FORMATOS PARA PAGOS DE CAMPAÑA USC - SE LE PAGA AL AUXILIAR, COLOCAR SOLO DATOS DEL AUXILIAR USC INCLUYENDO EL METODO DE PAGO
...
formato_pxl_abono_estado_cuenta <- archivo_consolidado_de_pagos %>% 
  filter(premio_x_logro=="Si",
         ...
         metodo_de_pago_asociado=="Abono a mi estado de cuenta de Asociado") %>% 
  mutate(fecha_de_solicitud={today()},
         valor_premio_x_logro=as.integer(valor_premio_x_logro),
         observacion="Premio x logro") %>% 
  select(cedula_referido,
         nombre_referido,
         cedula_asociado,
         nombre_asociado,
         valor_premio_x_logro,
         fecha_de_solicitud,
         observacion) %>% 
  rename(premio_asociado=valor_premio_x_logro) %>% 
  distinct(cedula_asociado, .keep_all = TRUE)
...
#CREAMOS UNA NUEVA TABLA SOLO CON LAS 3 COLUMNAS DEL FORMATO
tabla_formato_pxl_abono_estado_cuenta <- formato_pxl_abono_estado_cuenta %>% 
  select(cedula_asociado,
         nombre_asociado,
         premio_asociado,
         observacion)

#FORMATO FINAL CON TODAS LAS LIQUIDACIONES JUNTAS (LIQ. NORMAL - PREMIO X LOGRO - USC)
final_formato_abono_estado_de_cuenta=bind_rows(tabla_formato_abono_estado_de_cuenta, tabla_formato_pxl_abono_estado_cuenta)
...
final_formato_abono_estado_de_cuenta %>%
  write_xlsx(
    glue(
      "./output_data/",
      "Formato_Abono_Estado_De_Cuenta_",
      "{today()}.xlsx"
    )
  ) 


#EXPORTAMOS EL ARCHIVO ACTUALIZADO CON LAS CEDULAS DE PREMIO POR LOGRO
archivo_premio_x_logro %>%
  write_xlsx(
    glue(
      "./raw_data/",
...
formato_pxl_giros_y_consignaciones <- archivo_consolidado_de_pagos %>% 
  ...
  mutate(fecha_de_solicitud={today()},
         observacion="Premio x Logro",
         codigo_oficina=sucursal_pago_seleccionado_asociado,
         total_valor=valor_premio_x_logro,
         total_valor=as.integer(total_valor)) %>% 
  select(fecha_de_solicitud,
         cedula_asociado,
         nombre_asociado,
         total_valor,
...
#FORMATO FINAL CON TODAS LAS LIQUIDACIONES JUNTAS (LIQ. NORMAL - PREMIO X LOGRO - USC)
final_formato_giros_y_consignaciones=bind_rows(tabla_formato_giros_y_consignaciones, formato_pxl_giros_y_consignaciones)
...
final_formato_giros_y_consignaciones %>%
  write_xlsx(
    glue(
      "./output_data/",
      "Formato_Giros_y_Consignaciones_",
      "{today()}.xlsx"
    )
  ) 
...
#ARCHIVO FINANCIERO DEPURADO CONSOLIDADO DE PAGOS

liquidacion_financiera <- archivo_consolidado_de_pagos %>% 
  select(-indicativo_unico,
         -fecha_primera_contribuccion,
         -envio_sms,
         -solicitud_gestion_telemercadeo,
...
liquidacion_financiera %>%
  write_xlsx(
    glue(
      "./output_data/",
      "Formato_Liquidacion_Financiera_",
      "{today()}.xlsx"
    )
  )
...
#---------------------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------------------
# REALIZAMOS LA RENDERIZACION PARA GUARDAR LOS ARCHIVOS HTML

conflict_prefer("render", "rmarkdown")
# renderisamos los informes con la informacion actualizada
#render del informe principal de pagos

render("./html/informe_pagos.Rmd",
       output_file = glue("informe_pagos_",
                          "{today()}.html"),
       output_dir = "./html/")



# ENVIO CORREO - ABONOS ESTADOS DE CUENTA ----------------------------------------------------------------------
my_email_objet <- render_email('pagos_abonos_estados_de_cuenta.Rmd') %>% 
  add_attachment(glue("./output_data/Formato_Abono_Estado_De_Cuenta_","{today()}.xlsx"))
...
#          cc="","",
          subject="Pagos Red Cooperamos - Abonos Estados De Cuenta",
          credentials = creds_key("outlook"))


# ENVIO CORREO - GIROS Y CONSIGNACIONES ----------------------------------------------------------------------
my_email_objet1 <- render_email('pagos_giros_y_consignaciones.Rmd') %>% 
  add_attachment(glue("./output_data/Formato_Giros_y_Consignaciones_","{today()}.xlsx"))

class(my_email_objet1)

# codigo final para enviar el correo por medio del outlook
smtp_send(my_email_objet1,
          from = "yrhu0672@coomeva.com.co",
          to="yrhu0672@coomeva.com.co",
#          cc="","",
          subject="Pagos Red Cooperamos - Giros Y Consignaciones",
          credentials = creds_key("outlook"))


# ENVIO CORREO - LIQUIDACION FINANCIERA ----------------------------------------------------------------------
my_email_objet2 <- render_email('liquidacion_financiera.Rmd') %>% 
  add_attachment(glue("./output_data/Formato_Liquidacion_Financiera_","{today()}.xlsx"))

class(my_email_objet2)
...
smtp_send(my_email_objet2,
          ...
          to="yrhu0672@coomeva.com.co",
#          cc="","",
          subject="Consolidado - Liquidacion Financiera",
          credentials = creds_key("outlook"))


#SI NO SE LE COLOCA COPIA ALGUNA PERSONA Y SE LE DEJA SOLO LAS COMILLAS GENERA ERROR
...
#   odbc(),
#   driver = "SQL Server",
#   server = "PR-CDPWIN33,49345",
#   database = "DataMining",
#   encoding = "ISO-8859-1",
#   trusted_connection = "True"
# )
# 
# dbListFields(conn, "TaylorFISOL32_AsociadosEstadoCartera")
# data_server <- tbl(
#   conn,
#   in_schema("datamining", "TaylorFISOL32_AsociadosEstadoCartera")
# )
# data_local <- data_server %>%
#   filter(between(PERIODO==202204)) %>%
#   collect()
# data_local %<>%
