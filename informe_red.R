...
mes_vinculacion="Mes_Vinculacion"
mes_omitir_cierre="noviembre"

#EN EL CIERRE SE DEBE COLOCAR EL MES SIGUIENTE EN (MES_OMITIR_CIERRE)SOLO PARA EL CIERRE CUANDO SE VA GENERAR NORMAL ESTE DATO(MES_OMITIR_CIERRE) QUEDA VACIO
#CAMBIARMES - NOMBRE DONDE ESTA EL CODIGO QUE SE DEBE CAMBIAR
...
tinker_referidos <- dbGetQuery ( con , " SELECT DISTINCT NITCLI,FECING,ASOCIA,TIPCLI,TIPDOC,FECRET,AGCVIN,PROCOD,ESTASO,NUMINT,INDEST,ASOCOD
...
tinker_referidos <- tinker_referidos[with(tinker_referidos, order(tinker_referidos$referido_id_personal)), ]
tinker_referidos <- tinker_referidos[with(tinker_referidos, order(-tinker_referidos$referido_fecha_ing)), ]
...
#referido_codigo_unico=numint-------------------------------------------------------------------------------------------------
financiero_referido_conexion  <- dbGetQuery ( con , " SELECT DISTINCT NITCTA,CUOFAC,CUOCAN,CODPRO,FAPERT,FCANCE 
...
fecha_primera_contribuccion_conexion  <- dbGetQuery ( con , " SELECT CTANRO, MIN(FORIGE) as FEPRCO
...
tinker_referidos %<>%
  ...
  left_join(fecha_primera_contribuccion_conexion,by= c("codigo_primera_contribuccion" = "codigo_primera_contribuccion")) %>% 
...
crm_gestion %<>%
  ...
  mutate(fecha_de_creacion_6 = as.Date(fecha_de_creacion_6,format="%d/%m/%Y")) %>%
  arrange(desc(fecha_de_creacion_6)) %>%
  distinct(nombre_de_la_cuenta_numero_de_identificacion, .keep_all = TRUE)
...
#CRUZE CON LA BASE DE REFERIDOS EFECTIVOS Y QUE LE COLOQUE SI
...
#  left_join(
#    referidos_efectivos_2022, by = c("referido_id_personal" = "referido_id_personal")
#  ) %>% 
  mutate(
  ...
  mutate(
   ...
   # referido_fecha_ing = ymd(referido_fecha_ing),
    ...
    validacion_pago_asociado = if_else(
      (
        (tinker_nombre_estado %in% c("Activo Normal", "Activo Cobranza Cobranza Interna")) |
          colaborador == "Colaborador"
      ) &
        referido_efectivo == "Si" &
        referido_cuota_pagada >= 1 &
        (!tinker_id_personal %in% fuerza_comercial_mes[["cedula_ejecutivo_planta_lico"]]) &
        (!tinker_id_personal %in% planta_lico_sub) & #(PLANTA_LICO_SUB)-CARGOS DE LAS PERSONAS DE LA FUERZA DE VENTAS
        (!referido_cedula_promotor %in% planta_lico_tinker)&#(PLANTA_LICO_TINKER)-ASOCIADOS COOPERADORES
        fecha_primera_contribuccion!=is.na(fecha_primera_contribuccion),#QUE LA FECHA DEL PRIMER PAGO NO ESTE VACIA
      "Si",
      "No"
    ),
    validacion_pago_asociado = if_else(
  ...
  mutate(numero_llamadas_final=if_else(numero_llamadas!="","Si","No"),
         ...
         cono_estado_vinculacion=if_else(estado_vinculacion %in% c("Preingreso",
                                                                   "En proceso"),
                                         "Si","No"),
         regional_final=case_when(referido_regional!=""~referido_regional,
                                  regional_planta_lico!=""~regional_planta_lico,
 ...
 #        validacion_origen_referidos=case_when(fecha_de_creacion_referido_crm<referido_fecha_registro~"Fuerza Comercial",
  #                                             TRUE~"Call Center"),
         canal_vinculacion_referido=case_when(cargo_planta_lico=="AGENTE DE VINCULACIÓN CALL CENTER"~"Call Center",
                                              ...
                                              cargo_planta_lico %in% c("EJECUTIVO COMERCIAL MP",
                                                                       ...
                                                                       "EJECUTIVOS SALUD",
                                                                       "GERENTE DE ZONA",
                                                                       "COORDINADOR COMERCIAL CAMPAÑA CA",
                                                                       "CORREDOR DE SALUD",
  ...
  select(-codigo_primera_contribuccion,
         -fecha_primera_contribuccion)

# CRTL + SHIFT + C --- COLOCAR EN COMENTARIO LAS FILAS MULTIPLES
...
#CRUCE PARA REFERIDOS EFECTIVOS---------------------------------------------------------------------------------------
referidos_efectivos_2022_cruce <- referidos_efectivos_2022 %>%
  select(referido_id_personal,
         referido_efectivo) %>% 
  rename(referido_efectivo_final=referido_efectivo)

base_final %<>%
    left_join(
      referidos_efectivos_2022_cruce, by = c("referido_id_personal" = "referido_id_personal")
    )

  base_final%<>% 
  mutate(referido_efectivo=case_when(referido_efectivo_final=="Si"~"Si",
                                     TRUE~referido_efectivo)) %>% 
  select(-referido_efectivo_final)
...
tabla_dinamica_regional_registrados_mes_actual <- tabla_dinamica_regional_final1 %>%
  filter(Año_Registro==2022,Mes_Registro==mes_actual) %>% 
  select(Regional,
         Cantidad_Registrados)%>% 
  group_by(Regional) %>% 
...
tabla_dinamica_regional_registrados_acumulado <- tabla_dinamica_regional_final1 %>%
  filter(Año_Registro==2022,Mes_Registro!=mes_omitir_cierre) %>% #EN CIERRE SE DEBE COLOCAR ESTE CODIGO Mes_Registro!="noviembre" CAMBIARMES
  select(Regional,
         Cantidad_Registrados)%>% 
  group_by(Regional) %>% 
  summarise(Cantidad_Registrados=sum(Cantidad_Registrados))
...
tabla_dinamica_regional_efectivos_acumulado <- referidos_efectivos_2022 %>% 
  filter(ano_vinculacion==2022,Mes_Vinculacion!=mes_omitir_cierre) %>% #CAMBIARMES
...
tabla_dinamica_regional_preingresos_acumulado <- tabla_dinamica_regional_preingresos %>%
  filter(Año_Registro==2022,Mes_Registro!=mes_omitir_cierre) %>% #CAMBIARMES
  select(Regional,
         Cantidad_Preingresos)%>% 
  group_by(Regional) %>% 
  summarise(Cantidad_Preingresos=sum(Cantidad_Preingresos))
...
metas_red_cooperamos_cuadro_registrados <- read_excel(
  ...
  filter(mes=="enero"|mes=="febrero"|mes=="marzo"|mes=="abril"|mes=="mayo"|mes=="junio"|mes=="julio"|mes=="agosto"|mes=="septiembre"|mes==mes_actual) %>% 
...
### INFORMACION QUE VA EN EL REPORTE DE RED COOPERAMOS "INFORME RED COOPERAMOS RMD" -------------------------------------------------------------------
## RESUMEN ACUMULADO - 2022 -------------------------------------------------------------------------------------------------------------------
...
# TABLA PARA MOSTRAR ACUMULADO DE EFECTIVOS 2022
  ...
  (detalle_referidos_registrados_acumulados <- base_final %>% 
      ...
      rename(Cedula_Referente=tinker_id_personal,
             ...
             Año_Vinculacion=ano_vinculacion
      ))
...
#EFECTIVOS
cuadro_resumen_yen1 <- filter(referidos_efectivos_2022,ano_vinculacion==año_actual,Mes_Vinculacion==mes_actual) %>% 
  distinct(referido_id_personal, .keep_all = TRUE) %>% 
...
#PREINGRESOS
cuadro_resumen_yen2 <- filter(base_final,ano_registro==año_actual,estado_vinculacion=="En proceso"|estado_vinculacion=="Preingreso") %>% 
  count(canal_vinculacion_referido) %>% 
  rename(Indicador=canal_vinculacion_referido,
         Preingresos=n)
...
### INFORMACION QUE VA EN EL REPORTE DE RED COOPERAMOS "INFORME RED COOPERAMOS RMD" -------------------------------------------------------------------
## RESUMEN MENSUAL REGISTRADOS Y EFECTIVOS -------------------------------------------------------------------------------------------------------------------


#TOTAL REGISTRADOS MES ACTUAL
total_registrados_mes_actual <- cuadro_mensual_final %>% 
  filter(Regional=="Total") %>% 
  select(Cantidad_Registrados)

#TOTAL META REGISTRADOS MES ACTUAL
total_meta_registrados_mes_actual <- metas_red_cooperamos_nacional %>% 
  filter(Mes_Registro==mes_actual) %>% 
  select(Meta)

#TOTAL CUMPLIMIENTO MES ACTUAL
total_cumplimiento_registrados_mes_actual=percent(total_registrados_mes_actual$Cantidad_Registrados/total_meta_registrados_mes_actual$Meta)

#TOTAL EFECTIVOS MES ACTUAL
total_efectivos_mes_actual <- cuadro_mensual_final %>% 
  filter(Regional=="Total") %>% 
  select(Cantidad_Efectivos)


#TOTAL PREINGRESOS MES ACTUAL
total_preingresos_mes_actual <- cuadro_mensual_final %>% 
  filter(Regional=="Total") %>% 
  select(Cantidad_Preingresos)


#TOTAL VINCULACIONES MES ACTUAL
total_vinculaciones_mes_actual <- cuadro_mensual_final %>% 
  filter(Regional=="Total") %>% 
  select(Total_efectivos_y_Preingresos)


#TOTAL META EFECTIVOS MES ACTUAL
total_meta_efectivos_mes_actual <- cuadro_mensual_final %>% 
  filter(Regional=="Total") %>% 
  select(Meta)

#TOTAL CUMPLIMIENTO EFECTIVOS MES ACTUAL
total_cumplimiento_efectivos_mes_actual <- cuadro_mensual_final %>% 
  filter(Regional=="Total") %>% 
  select(Cumplimiento)
...
# BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC  - BASE CAMPAÑA USC - BASE CAMPAÑA USC
# BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC - BASE CAMPAÑA USC  - BASE CAMPAÑA USC - BASE CAMPAÑA USC


# para exportar las bases en diferentes hojas en el mismo libro es necesario

archivoo="Base_Campaña_USC_"
hoyy={today()}
extt=".xlsx"
...
base_final_usc <- base_final %>% 
  ...
  mutate(Val_Fecha_junio_Comienzo_Campaña_REGISTRO=if_else(referido_fecha_registro>="2021-06-20","Si","No"),
         Tipo_De_Comunicacion=case_when(nombre_1.x!=is.na(nombre_1.x)&Val_Fecha_junio_Comienzo_Campaña_REGISTRO=="Si"~"Comunicación Cliente o Usuario",
                                        nombre_referido!=is.na(nombre_referido)&Val_Fecha_junio_Comienzo_Campaña_REGISTRO=="Si"~"Comunicación Asociado"),
         ...
         Campaña_Auxiliar=case_when(Tipo_De_Comunicacion=="Comunicación Cliente o Usuario"~campana_costeo.x,
...
# Exportamos la base final --------------------------------------------------- CAMPAÑA USC
...
base_fc_prospectos_referidos <- crm_exportar %>% 
  filter(ano_registro_crm==2022) %>% 
  rename(nombre_campaña=origen_de_la_campana_principal_nombre_de_la_campana,
...
base_final_mp <- base_final%>%
  ...
  filter(nombre_completo!=is.na(nombre_completo),ano_registro==2022,Mes_Registro=="abril"|Mes_Registro=="mayo"|Mes_Registro=="junio"|Mes_Registro=="julio"|Mes_Registro=="agosto"|Mes_Registro=="septiembre"|Mes_Registro==mes_actual) %>% 
...
# BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP
# BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP - BASE CALL WHATSAPP

archivooooo="Base_Referidos_Call"
hoyyyyy={today()}
exttttt=".xlsx"
nombreeeee_archivo=paste0(archivooooo,hoyyyyy,exttttt)

rutaaaaa="C:/Users/yrhu0672/Desktop/R/Informe - Red Cooperamos 2.0 - 2022/output_data/"
ruta_completaaaaa=paste0(rutaaaaa,nombreeeee_archivo)

base_call_whatsapp <- base_final %>% 
  select(validacion_origen_referidos,
         referido_id_personal,
         referido_nombre,
         referido_fecha_registro,
         tinker_id_personal,
         ...
         fecha_primera_marcacion,
         preselecta,
         fecha_de_creacion_referido_crm,
         cedula_ejecutivo_crm,
         regional_registrada_crm,
         tipo_de_tarea_crm,
         tipo_de_respuesta_crm,
...
referidos_efectivos_call_whatsapp <- referidos_efectivos_2022 %>% 
  select(validacion_origen_referidos,
         referido_id_personal,
         referido_nombre,
         referido_fecha_registro,
...
# para exportar las bases en diferentes hojas en el mismo libro es necesario
sheets5 <- list("Base_Call_Whatsapp" = base_call_whatsapp, "Registrados_Fuerza_Ventas" = base_fc_prospectos_referidos, "Referidos_Efectivos"= referidos_efectivos_call_whatsapp) #assume sheet1 and sheet2 are data frames
write_xlsx(sheets5, ruta_completaaaaa)
